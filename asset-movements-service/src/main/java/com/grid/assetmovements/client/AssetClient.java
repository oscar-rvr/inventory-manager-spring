...
public class AssetClient {
    ...
    public void validateAssetExists(Long assetId) {
        ...
        webClient.get().uri(url).retrieve().onStatus(status -> status.is4xxClientError(), response -> Mono.error(new AssetNotFoundException("id: " + assetId))).bodyToMono(Void.class).block();
    }
}
